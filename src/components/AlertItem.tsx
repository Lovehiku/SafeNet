...
export function AlertItem({ alert, onClick }: Props) {
  return (
    <motion.button
      onClick={onClick}
      className="w-full text-left card-surface p-4 hover:border-primary-400 transition-colors"
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="flex flex-wrap gap-2 mt-2 text-xs text-muted">
            <span className="px-2 py-1 rounded-full bg-gray-50 border border-gray-200">{alert.source}</span>
            <span className="px-2 py-1 rounded-full bg-gray-50 border border-gray-200">
              {new Date(alert.date).toLocaleString()}
            </span>
            <span className="px-2 py-1 rounded-full bg-gray-50 border border-gray-200 capitalize">{alert.status}</span>
          </div>
        </div>
      </div>
    </motion.button>
